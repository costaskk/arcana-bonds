...
export default function Portrait({ id, glyph = "✨", rarity = "B", size = 96, className = "" }) {
  ...
  const ring =
    rarity === "SS"
      ? "from-yellow-200 via-fuchsia-300 to-cyan-300"
      : rarity === "S"
  ...
  if (!errored) {
    return (
      <div className={`relative inline-block ${className}`} style={{ width: size, height: size }}>
        ...
        {!loaded && (
          <div className="absolute inset-0 rounded-full animate-pulse bg-slate-200/60 dark:bg-slate-800/60" />
        )}
        <motion.img
  ...
  return (
    <div className={`relative inline-block ${className}`} style={{ width: size, height: size }}>
      ...
      >
        <span style={{ fontSize: size * 0.45 }} aria-hidden>
          {glyph}
        </span>
      </motion.div>
    </div>
