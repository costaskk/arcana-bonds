  ...
  return (
    <div className="p-3 rounded-xl border bg-white/70 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800">
      ...
      <div className="mt-2 grid grid-cols-[auto,1fr] gap-3 items-center">
        <Portrait id={unit.id} glyph={unit.portrait || unit.emoji} rarity={unit.rarity} size={88}/>
        <div className="relative">
          <AnimatedClip id={unit.id} type="idle" width={220} height={220} />
          {/* If idle clip missing, we show emoji as soft fallback */}
      ...
      <div className="mt-2 flex flex-wrap gap-1 text-xs">
        {unit.skills.map((s)=>(<span key={s} className="px-2 py-0.5 rounded-full bg-slate-900/5 dark:bg-white/10">{s}</span>))}
      </div>
